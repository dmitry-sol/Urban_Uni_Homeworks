...
class UrTube:
    ...
    def add(self, *videos):
        for video in videos:
            if video.title in str(self.videos):
                ...
                if title == str(video):
                    if not video.adult_mode or (self.current_user_age >= 18 and video.adult_mode == True):
                        ...
                        while video.time_now < video.duration:
                            video.time_now += 1
                            if video.time_now % 30 == 0:
                                newline_sequence = '\n'
                            else:
                                newline_sequence = ' '
                            print(video.time_now, newline_sequence, end='')
                            t.sleep(0.5)
                        print(' = Конец видео =')
                        video.time_now = 0
                    else:
